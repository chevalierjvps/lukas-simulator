    ...
    return (
        <>
            {/* Floating Action Buttons — horizontal strip sitting above
                ...
                fabAlign 'seam' (chat layout) starts the strip at the column
                seam so it lies over the vitals monitor, never the chat;
                'left' (full-surface plugin rooms like the 3D room) docks it
                at the very left so it never covers the room's own
                bottom-center surfaces. */}
            {!isOpen && (
                <div
                    className="fixed z-[45] flex gap-2"
                    style={{ bottom: '250px', left: fabAlign === 'left' ? '1rem' : 'calc(max(35vw, 350px) + 1rem)' }}
                >
                    {tabs.map(tab => (
                        <button
                            key={tab.id}

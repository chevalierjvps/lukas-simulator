        ...
        <div className={bare
            ...
            <div className={stacked ? 'flex flex-col gap-4' : 'flex gap-4'}>
                ...
                {stacked ? (
                    ...
                    {/* Audio Player. The compact transport (real recorded
                        audio only — live synth has no fixed duration to
                        seek) is one line: tiny play, a real seek bar,
                        elapsed/length, for panels that cannot spare a 40px
                        control block. Otherwise the full-size player, which
                        also drives the live stethoscope synth when no
                        recorded clip exists for this point. */}
                    {(() => {
                        if (currentAudioUrl && transport === 'compact') {
                            return (
                                <div className="rounded-lg bg-slate-900 px-2 py-1.5">
                                    <div className="mb-1 truncate px-1 text-[10px] text-slate-400">
                                        {currentPoint ? t('point_sounds', { point: t(currentPoint.labelKey) }) : t(profile.playerLabelKey)}
                            ...
                            <div className="bg-slate-900 rounded-lg p-3">
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={togglePlay}
                                        className="w-10 h-10 rounded-full bg-cyan-600 hover:bg-cyan-500 flex items-center justify-center transition-colors"
                                        // aria-label, not title: `title` doubles here as the
                                        // "this is an auscultation point" marker every test
                                        // (and the point buttons themselves) query on
                                        // (`button[title]`) — a hover tooltip on THIS button
                                        // would silently get counted as a tenth point. The
                                        // same "ao vivo" info is already visible on the badge
                                        // beside it, with its own title.
                                        aria-label={usingSynth ? t('replay_live_sound', { defaultValue: 'Repetir som ao vivo' }) : undefined}
                                    >
                                        {isPlaying ? (
                                            <Pause className="w-5 h-5 text-white" />
                                        ) : (

...
describe('usePhysicalExam', () => {
    ...
    it('says "Não examinado" for a region the model does not know', () => {
        // This fork's getDefaultFinding defaults to PT-BR, not English, when
        // no case language is given — the product is PT-BR-first.
        const entry = perform(null, 'notARegion', 'palpation');
        expect(entry.finding).toBe('Não examinado');
        expect(examined).toHaveBeenCalledWith('notARegion', 'palpation', 'Não examinado');
